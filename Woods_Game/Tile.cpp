...
Tile::Tile(TileSet *tileset, int tile_x, int tile_y, int type_index, int sheet_col, int sheet_row)
{
	...
	Register("TileEdges", &tile_edges);
	//Register("Block", &block);
	this->rect.x = tile_x * TILE_SIZE;
	this->rect.y = tile_y * TILE_SIZE;
...
void Tile::reset(TileSet *tileset, Tile * t, const int tile_x, const int tile_y)
{
...
void Tile::draw(ALLEGRO_DISPLAY *display, const int x_offset, const int y_offset, const int screen_w, const int screen_h)
{
	GameImage::draw(display, x_offset, y_offset, screen_w, screen_h);
}

void Tile::draw_block(ALLEGRO_DISPLAY *display, const int x_offset, const int y_offset, const int screen_w, const int screen_h)
{
	if (!block.is_empty() && block.is_visible()) {
		block.draw(display, x_offset, y_offset, screen_w, screen_h);
	}
}
...
void Tile::replace_block(TileSet * tileset, Block * other_block, std::pair<int, int> ss_pos, std::pair<int, int> tpos)
{
	...
	const bool visible = other_block->is_visible();
	this->block.set_entity_data_index(other_block->get_entity_data_index());
	this->block.set_content(filename, offset_rect, pixel_pos);
	this->block.set_starting_pos(pixel_pos.first, pixel_pos.second);
	this->block.set_entity_sheet_offset(ss_pos.first, ss_pos.second);
...
{
	this->rect.x = x * TILE_SIZE;
}

void Tile::set_tile_pos_y(int y)
{
	this->rect.y = y * TILE_SIZE;
}
...
{
	return (int) this->get_x() / TILE_SIZE;
}

int Tile::get_tile_pos_y()
{
	return (int)this->get_y() / TILE_SIZE;
}
