...
Critter::Critter(CritterTemplate * critter_template)
{
...
void Critter::request_pathing_update(World * world, Level * level, GlobalTime * time)
{
	...
	if (this->ai_state.is_idle()) {
		...
		//TEMP-- want to have different critters behave differently, (different AI types in library?) but probably keep code universal as appropriate
		const int range = 1; //temp
